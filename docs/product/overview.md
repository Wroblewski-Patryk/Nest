...
- Online-first with caching and robust synchronization, including offline
  ...
  and stops on first error with explicit user-facing reason. Retry starts from
  queue beginning and uses idempotent checks to skip already-synced items.
  Success feedback follows the same standard notification style as other
  successful actions.
